...
def augment_dataset(image_dir, label_dir, output_image_dir, output_label_dir,
                    class_map_path=None, augmentations_per_image=3, rare_boost_factor=2):
    ...
    print(f"[INFO] Rare classes: {rare_classes}")

    rare_class_transform = A.Compose([
        A.RandomResizedCrop(height=640, width=640, scale=(0.7, 1.0), ratio=(0.8, 1.2), p=0.8),
    ...
    common_class_transform = A.Compose([
    ...
    for fname in os.listdir(image_dir):
        ...
        # Rare class condition
        contains_rare = any(c in rare_classes for c in class_labels)
        transform = rare_class_transform if contains_rare else common_class_transform
            ...
            try:
                ...
                with open(out_lbl_path, "w") as f:
                    for cid, box in zip(transformed_labels, transformed_bboxes):
                        cid = int(cid)
                        x, y, w, h = [np.clip(coord, 0.0, 1.0) for coord in box]
...
def swap_folders(folder_pairs):
    """
    Swaps folder names between pairs using temporary names to avoid conflicts.
    
    Args:
        folder_pairs (list of tuples): List of (original_folder, target_folder) pairs to swap.
    """
   
    for orig, _ in folder_pairs:
        temp_name = f"{orig}_temp"
        if os.path.exists(orig):
            os.rename(orig, temp_name)
            print(f" Renamed '{orig}' → '{temp_name}'")
        else:
            print(f"Folder not found: {orig}")


    for orig, target in folder_pairs:
        if os.path.exists(target):
            os.rename(target, orig)
            print(f"Renamed '{target}' → '{orig}'")
        else:
            print(f"[WARNING] Folder not found: {target}")

    for orig, target in folder_pairs:
        temp_name = f"{orig}_temp"
        if os.path.exists(temp_name):
            os.rename(temp_name, target)
            print(f"Renamed '{temp_name}' → '{target}'")
        else:
            print(f"[WARNING] Temp folder not found: {temp_name}")
...
def check_image_label_consistency(image_dir, label_dir, class_map_path, report_path="mismatched_files.txt"):
    ...
    for label_id_str, entry in class_map.items():
        ...
        class_id_to_label_id[class_id] = label_id_str 
...
def merge_into_folder(folder_a, folder_b):
    """
...
def remap_yolo_labels(
    ...
    # Load label.labels (index → category_id)
    with open(labels_txt_path, "r") as f:
        label_lines = f.read().splitlines()

    label_index_to_category = {
        str(i): int(line.strip()) for i, line in enumerate(label_lines)
    }

    # Load category_id → new class_id map
    category_to_new_class_id = {
        int(v["class_id"]): int(k) for k, v in json_map.items()
    }

    # Prepare output folder
    if output_dir is None:
        output_dir = annotations_dir  # Overwrite
    else:
        os.makedirs(output_dir, exist_ok=True)
    ...
    for file in os.listdir(annotations_dir):
        ...
        in_path = os.path.join(annotations_dir, file)
        ...
        for line in lines:
            ...
            idx = parts[0]  # original index from YOLO file
            rest = parts[1:]

            # Step 1: Convert index → category_id
            if idx not in label_index_to_category:
                print(f" Unknown index: {idx} in {file}")
                continue

            category_id = label_index_to_category[idx]

            # Step 2: Convert category_id → new class_id
            if category_id not in category_to_new_class_id:
                print(f"category_id {category_id} not found in json map (file: {file})")
                continue

            new_class_id = category_to_new_class_id[category_id]
    ...
    print(f"\n🎉 All annotations updated and saved to: {output_dir}")
